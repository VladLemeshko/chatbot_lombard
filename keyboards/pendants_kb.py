...
def create_filter_keyboard_pendants(user_filters):
    ...
    keyboard.add(
        InlineKeyboardButton(f"{price_text}", callback_data="filter_price_pendants"),
        InlineKeyboardButton(f"{brand_text}", callback_data="filter_brand_pendants"),
        InlineKeyboardButton(f"{condition_text}", callback_data="filter_condition_pendants"),
        InlineKeyboardButton(f"{gender_text}", callback_data="filter_gender_pendants")
    )

    # Кнопка "Сбросить фильтры"
    reset_button = InlineKeyboardButton("Сбросить фильтры", callback_data="reset_filters_pendants")
    keyboard.add(reset_button)

    keyboard.add(InlineKeyboardButton("Поиск🔎", switch_inline_query_current_chat="pendants"))
