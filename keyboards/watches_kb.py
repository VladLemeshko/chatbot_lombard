...
def create_filter_keyboard(user_filters):
    ...
    keyboard.add(
        InlineKeyboardButton(f"{price_text}", callback_data="filter_price"),
        InlineKeyboardButton(f"{brand_text}", callback_data="filter_brand"),
        InlineKeyboardButton(f"{condition_text}", callback_data="filter_condition"),
        InlineKeyboardButton(f"{gender_text}", callback_data="filter_gender")
    )

    # Кнопка "Сбросить фильтры"
    reset_button = InlineKeyboardButton("Сбросить фильтры", callback_data="reset_filters")
    keyboard.add(reset_button)

    keyboard.add(InlineKeyboardButton("Поиск🔎", switch_inline_query_current_chat="watches"))
