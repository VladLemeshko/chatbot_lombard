...
db_path_earrings = os.path.join(earrings_data_dir, "earrings.db")

# Асинхронная функция для создания соединения с базой данных и получения курсора
async def create_connection_earrings():
    conn = sqlite3.connect(db_path_earrings)
    cursor = conn.cursor()
    return conn, cursor

# Асинхронная функция для закрытия соединения с базой данных
async def close_connection_earrings(conn):
    conn.commit()
    conn.close()
...
async def create_settings_table_earrings():
...
async def update_setting_earrings(name, value):
    conn, cursor = await create_connection_earrings()
    try:
...
async def get_setting_earrings(name):
    conn, cursor = await create_connection_earrings()
    try:
...
async def set_flag_earrings(flag_name, value):
    await update_setting_earrings(flag_name, value)

async def get_flag_earrings(flag_name):
    return await get_setting_earrings(flag_name)

async def get_total_earrings():
    total_earrings = await get_setting_earrings("total_earrings")
    if total_earrings is None:
        return 0
    else:
        return int(total_earrings)
    
async def update_total_earrings(total_earrings):
    await update_setting_earrings("total_earrings", str(total_earrings))
    


# Асинхронная функция для создания таблицы "earrings"
async def create_table_earrings():
    conn, cursor = await create_connection_earrings()
    try:
        create_table_query = """
...
async def clear_table_earrings():
    conn, cursor = await create_connection_earrings()
    try:
        cursor.execute("DELETE FROM earrings")
        conn.commit()
    finally:
        await close_connection_earrings(conn)
...
async def insert_earrings_details(**kwargs):
    conn, cursor = await create_connection_earrings()
    try:
        insert_query = """
        INSERT OR REPLACE INTO earrings (manufacture, name, price, for_whom, conditions, images, characteristics, link)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
        values = (
...
async def get_price_range_earrings():
    conn, cursor = await create_connection_earrings()
    try:
        cursor.execute("SELECT MIN(CAST(REPLACE(REPLACE(price, ',', ''), '$', '') AS INTEGER)), MAX(CAST(REPLACE(REPLACE(price, ',', ''), '$', '') AS INTEGER)) FROM earrings")
...
async def get_unique_brands_earrings():
    conn, cursor = await create_connection_earrings()
    try:
        cursor.execute("SELECT DISTINCT manufacture FROM earrings WHERE manufacture IS NOT NULL")
...
async def get_all_earrings(limit=None, offset=None):
    conn, cursor = await create_connection_earrings()
    try:
        query = "SELECT id, manufacture, name, price, for_whom, conditions, images, characteristics FROM earrings"
...
async def get_filtered_earrings(filters, limit=None, offset=None):
    conn, cursor = await create_connection_earrings()
    try:
        params = []
        where_conditions = []
...
async def get_earrings_data_by_id(watch_id):
    conn, cursor = await create_connection_earrings()
    try:
        cursor.execute("SELECT id, manufacture, name, price, for_whom, conditions, images, characteristics FROM earrings WHERE id = ?", (watch_id,))
        earrings_data = cursor.fetchone()
        return earrings_data
    finally:
        await close_connection_earrings(conn)