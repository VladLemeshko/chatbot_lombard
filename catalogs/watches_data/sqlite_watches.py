...
db_path = os.path.join(watches_data_dir, "watches.db")
...
async def set_flag(flag_name, value):
    await update_setting(flag_name, value)

async def get_flag(flag_name):
    return await get_setting(flag_name)

async def get_total_watches():
    total_watches = await get_setting("total_watches")
    if total_watches is None:
        return 0
    else:
        return int(total_watches)
    
async def update_total_watches(total_watches):
    await update_setting("total_watches", str(total_watches))
    


# Асинхронная функция для создания таблицы "watches"
...
async def insert_watch_details(**kwargs):
    conn, cursor = await create_connection()
    try:
        insert_query = """
        INSERT OR REPLACE INTO watches (manufacture, name, price, for_whom, conditions, images, characteristics, link)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
        values = (
...
async def get_price_range():
    conn, cursor = await create_connection()
    try:
        cursor.execute("SELECT MIN(CAST(REPLACE(REPLACE(price, ',', ''), '$', '') AS INTEGER)), MAX(CAST(REPLACE(REPLACE(price, ',', ''), '$', '') AS INTEGER)) FROM watches")
...
async def get_unique_brands():
    conn, cursor = await create_connection()
    try:
        cursor.execute("SELECT DISTINCT manufacture FROM watches WHERE manufacture IS NOT NULL")
...
async def get_all_watches(limit=None, offset=None):
    conn, cursor = await create_connection()
    try:
        query = "SELECT id, manufacture, name, price, for_whom, conditions, images, characteristics FROM watches"
...
async def get_filtered_watches(filters, limit=None, offset=None):
    conn, cursor = await create_connection()
    try:
        params = []
        where_conditions = []
...
async def get_watch_data_by_id(watch_id):
    conn, cursor = await create_connection()
    try:
        cursor.execute("SELECT id, manufacture, name, price, for_whom, conditions, images, characteristics FROM watches WHERE id = ?", (watch_id,))
